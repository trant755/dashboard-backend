...
const monitoringDistrict = async (req, res) => {
  const { district } = req.params;

  try {
    const result = await getRowCount(
      `SELECT * FROM monitoring WHERE district LIKE '%${district}%'`
    );
    ...
    console.log("let osvita_obstezheno", osvita_obstezheno);
    ...
    result.forEach((row) => {
      ...
      if (row.admin_usuneno !== "-" && row.admin_usuneno !== "") {
        admin_usuneno += parseInt(row.admin_usuneno);
      }

      if (row.soc_obstezheno !== "-" && row.soc_obstezheno !== "") {
        soc_obstezheno += parseInt(row.soc_obstezheno);
      }
...
module.exports = { monitoringDistrict };
